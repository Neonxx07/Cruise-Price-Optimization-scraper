...
CruiseIntel Optimization is an internal repricing-intelligence system built for a cruise travel agency. It automates a task travel agents otherwise do by hand: log into a cruise line's booking portal, look up a client's reservation, check whether a cheaper cabin category or an addable discount has become available, and figure out whether taking it is *actually* a good deal once onboard credit (OBC) changes, lost packages/perks/fares, and MSC-specific rules like final-payment-date cutoffs are accounted for. The system covers four cruise-line portals: Royal Caribbean/Celebrity's **ESPRESSO** (`cruisingpower.com`), Norwegian Cruise Line's **SeaWeb Agents** portal (`seawebagents.ncl.com`), Carnival Cruise Line's **GoCCL Navigator** portal (see [Known Open Issues](#known-open-issues) — this document does not yet have a full GoCCL reference section), and **MSC Cruises**, a structurally different problem (price-match, discount-add, discount-tier-upgrade, and Voyagers-Club-selection opportunities, not a category-upgrade search — see [MSC Cruises Reference](#msc-cruises-reference)) with its own scraper, calculator, and GUI integration. ESPRESSO/NCL/GoCCL reads the current price and inclusions for a booking, probes for a cheaper category, and runs a shared net-savings formula to classify the opportunity; the monorepo contains **two independent, functionally-equivalent implementations** of that logic — a Chrome MV3 browser extension (JavaScript) for quick single-agent checks, and a Python/Playwright platform (CLI, desktop GUI, and FastAPI server) for batch processing and automation — sharing the same net-saving formula, seven-way classification model, and confidence scoring, so a booking checked by either tool produces the same verdict. **Hard project-wide rule: no AI/LLM/API keys anywhere in this system, ever — not even for one-time, human-supervised tooling** (see the end of [MSC Cruises Reference](#msc-cruises-reference) for the full rationale and the classical techniques — RapidFuzz, ARIA-snapshot heuristics, `mlxtend`, real parsing libraries — used in their place).
...
**⚠ KNOWN DEFECT in the `lost_fobc` check, confirmed against real data 2026-08-26 and deliberately NOT yet patched:** the literal substring `"FOBC"` does not match the real promo codes involved. On real booking 3000007 the promos went from `FREESRVC` (before) to `FITOBC` (after) — and neither contains `"FOBC"` as a substring (`FITOBC` is F-I-T-O-B-C; the four-character window is `TOBC`, not `FOBC`). So this detection **silently misses the exact real scenario it was written for**. Worse, the observed direction is the *opposite* of what the code assumes: the OBC-certificate addon is **gained**, not lost, when the non-OBC `FREESRVC` perk disappears. Fixing the string alone would therefore be wrong — whether an addon *swap* like this should reduce `net_saving` at all is an open business question (the project owner's own reference report leaves the dollar verdict as the raw price difference, unadjusted). See the [NCL Reference](#norwegian-cruise-line-ncl-reference)'s promo section; `_summarize_addon_change` surfaces the real swap informationally in the meantime rather than guessing at the dollar math.

**Additionally, `calculate_ncl` now applies a hard protected-promo gate BEFORE any status is assigned** — see [HARD RULE: never reprice away a protected promo (LATRIPLE)](#norwegian-cruise-line-ncl-reference). A booking that would lose `LATRIPLE` returns `TRAP` regardless of how large the price drop is.
...
> **⚠ THIS EXTENSION PATH IS KNOWN-INCORRECT as of 2026-08-26.** The switch-to-a-different-cheaper-category model described above was proven wrong against the real portal — NCL repricing is a **same-category** reprice. The Python platform's `scraper/ncl.py` was corrected; this extension adapter was **not**, so the two implementations no longer agree on NCL and the monorepo's usual "either tool produces the same verdict" guarantee does not hold for this cruise line. The extension also lacks all three dialog fixes and the SlickGrid-virtualization fix. See [Norwegian Cruise Line (NCL) Reference](#norwegian-cruise-line-ncl-reference) for what correct behavior actually is. Treat extension NCL results as unreliable until this is brought in line.
...
**MSC support, added 2026-08-24**: `self.msc_service = MscLiveService()` runs alongside `GuiScanAdapter`/`BookingQueueManager` (MSC's own architecture doesn't reuse the ESPRESSO/NCL `BookingResult` pipeline — see [MSC Cruises Reference](#msc-cruises-reference)). MSC was re-added to the `cruise_line_selector` dropdown; `_on_login_check`/`_on_start` branch per selected cruise line, routing to new `_run_msc_batch`/`_append_msc_result_row` methods for MSC and the existing path otherwise; `_color_for_status` and `_on_export` were extended to also handle `MscBookingResult`. See [MSC in the desktop GUI](#msc-cruises-reference) for the details, including a real stale-process incident this integration surfaced.
...
Two bookings (3000034, 3000033) failed a full batch run with `#reservationid`-visibility timeouts, and both turned out to be **group bookings** (page title "Group Booking Summary" instead of the individual-booking "Reservation Summary"). That correlation looked causal at first — worth investigating rather than assuming, per this project's standing rule of checking captured data before concluding anything. The captured snapshot for 3000034 showed one real oddity: the top-of-page reservation-status widget's Angular binding never rendered (literal `{{sb.reservation.status...}}` template text instead of a real value), while everything else on the same page — price, guests, offers — rendered correctly. On the very next retry, the session briefly bounced to `/login` entirely.

**Investigated on the next full run, with zero code changes**: both bookings went through the identical flow used for individual bookings — `_read_category()` (`#currentPriceCat`), click "Categories", read the category table, allocate + `showRepriceModalCheck` — and produced clean, correct, real results: 3000034 came back `NO_SAVING`/"Booking restriction — price program change not allowed" (the existing skip_reprice sentinel), and 3000033 came back `WLT`/"WLT - waitlisted" (the existing WLT sentinel). **Conclusion: group bookings are already fully supported by the existing flow — no special-casing needed.** The original failure was an isolated `Failed to fetch` network error on the reprice API call for 3000034, which then cascaded into the next booking's search also timing out — ordinary transient noise (the same category as any other isolated `ERROR` result), not a structural incompatibility with group-booking pages. The broken top-status widget is purely cosmetic display and isn't read by any scraper logic, so its rendering failure has no functional effect either way.

### 8. SELF-CAUGHT NEAR-MISS: "hardening" ESPRESSO's row-price parser with a real parsing library made it WORSE, not better

2026-08-24: while adopting `price-parser`/`dateparser`/RapidFuzz elsewhere in the project (see [MSC Cruises Reference](#msc-cruises-reference)), the same treatment was attempted on ESPRESSO's `_price_from_row` (`core/calculator.py`, backs `find_upgrade_candidates` — one of this project's highest-value features), replacing the narrow `_ROW_PRICE_RE = re.compile(r"([\d,]+\.\d{2})")` regex with `price_parser.Price.fromstring()` run against the row's full `rowText`. This looked like a strict improvement — a maintained parsing library instead of a hand-rolled regex. **Verifying it against all 83,296 already-captured real ESPRESSO category rows before trusting it (this project's standing rule) proved it was actually a regression**: `Price.fromstring()`, given the full messy multi-field row text, latched onto OTHER bare numbers embedded in unrelated fields (e.g. `"WLT(0)"`, `"AVL(2)"`) instead of the real price — a genuine $8,782.00 Royal Suite row parsed as $0.00. **Fully reverted** to the original exact-decimal-shape regex, which turns out to be the correct disambiguator for this specific case precisely because the row text is NOT already isolated to a single value (unlike MSC's/GoCCL's price fields, where the same library-based approach shipped cleanly). **Lesson: "use a real library instead of a regex" is not a universal improvement — verify the new approach against real captured data for the specific field shape in front of you, every time, even when the change looks obviously safe.**

### 9. A long-running GUI process kept executing stale code after a fix shipped — MSC batch checks failed silently for days

2026-08-24: booking 3000030 kept showing a 6-day-old, incorrect green (opportunity) result in the desktop GUI after the real fix (senior discount requires 2+ seniors, not 1 — see [MSC Cruises Reference](#msc-cruises-reference)) had already shipped to `msc_commands.py`, and a live 26-booking GUI batch run failed on every single booking with `TypeError: evaluate_msc_booking() got an unexpected keyword argument 'all_seniors'` — a parameter that had already been renamed to `senior_count`. **Root cause**: `services/msc_live_service.py` holds one Python process open for the lifetime of a GUI session, and Python had already imported `msc_commands` into memory before the fix landed on disk — editing a `.py` file does not change what an already-running process is executing. The console-based `msc_session_controller.py` had never had this problem because it already re-imports the module fresh per command. **Fix**: added `importlib.reload(msc_commands)` at the top of both `MscLiveService.check_login()` and inside `run_batch()`'s per-booking loop, so the GUI process always runs the current on-disk code. **Lesson: any long-running process that imports project code needs an explicit reload strategy, or a stale-looking result is a real signal worth checking against the running process's actual start time — not assumed to be a fluke or a data problem.**

### 10. "GRP TVL PRTC" — a real travel-protection line item was being silently blended into ordinary "Lost Packages"

2026-08-25: an audit for gaps a human agent would catch but the pipeline wouldn't led to mining all 532 captured real ESPRESSO invoice responses (`data/raw_responses.jsonl`) for every distinct invoice-item name ever seen. Exactly one travel-protection-shaped item turned up: `"GRP TVL PRTC"` (almost certainly "Group Travel Protection"), 64 real occurrences — never in `ESPRESSO_FEE_TYPES`, so it fell straight through `_get_packages()` and was indistinguishable from a lost drink package or Wi-Fi perk in `lost_pkg_names`. Losing trip-insurance/travel-protection coverage is a materially different, more consequential kind of loss than losing a bundled perk (it can affect cancellation/refund eligibility), so a human reviewing the same invoice would flag it differently — the pipeline didn't. **Fix**: `_is_travel_protection()`/`_TRAVEL_PROTECTION_PATTERNS` (`core/calculator.py`) classify it into a new, separate `BookingResult.lost_travel_protection` field (`core/models.py`) with its own note fragment ("ALSO LOSES TRAVEL PROTECTION... confirm this doesn't affect cancellation/trip-insurance coverage before repricing") — the dollar amount still contributes to `lost_pkg_value`/`net_saving` exactly as before (no financial-math change), only the NAME is pulled out of the generic `lost_pkg_names` bucket. CSV/Excel exports both gained a "Lost Travel Protection" column for parity. The pattern list includes a few unconfirmed synonyms (industry-standard travel-protection product naming) alongside the one confirmed-real string, following this project's established practice of building a little defensively around one real example rather than coding to only the exact string seen so far.

### 11. Investigated and found already-safe: cancelled bookings, the ESPRESSO c3 rate column, and GoCCL's guest count

Same 2026-08-25 gap audit, three findings worth recording so they aren't "rediscovered" as bugs later:

- **A cancelled/postponed ESPRESSO or GoCCL booking does NOT currently produce a nonsense OPTIMIZATION/TRAP the way a real MSC incident (booking 3000012) once did.** Checked directly against real data: across all 3,987 real captured ESPRESSO bookings, every single one with `old_total` or `new_total` at or near $0 landed in `NO_SAVING`/`WLT`/`PAID_IN_FULL`/`ERROR` — zero contamination into `OPTIMIZATION`/`TRAP`. The same holds structurally for `calculate_goccl`: a $0 `current_price_gross` makes `price_drop` negative, which also routes to `NO_SAVING`. This is NOT the same failure shape as MSC's incident — MSC's vulnerability came specifically from comparing against a separate DUMMY booking's data rather than the real booking's own current total; ESPRESSO/NCL/GoCCL's math is always anchored to the same live booking's own real total, which is why the degenerate-zero case already routes safely. No fix was built here because none is needed — confirmed, not assumed.
- **ESPRESSO's second rate-program column ("c3") is correctly and deliberately still unevaluated.** `_capture_category_table` (`scraper/espresso.py`) has captured both `c2Cells`/`c3Cells` since 2026-08-13, but per that code's own docstring, no live evidence has ever confirmed what a `c3` price actually means or how it compares to `c2` — comparing them would be guessing, the exact mistake the free-upgrade-detection incident already warned against. Checked directly: across 2,168 real `market_data` captures, **zero** show `dualRateColumns: true` — Neon's observed "sometimes the second column is cheaper" scenario has never actually been captured yet, so there still isn't a real evidence base to build the comparison on. Correctly deferred; not a gap to close today.
- **GoCCL's guest-count blind spot is real but has no verifiable fix yet** — no live GoCCL capture has ever confirmed what field (if any) holds a real per-booking occupancy count in `window.initialData`, so building a real fix would mean guessing a selector, which this project doesn't do. Instead: `_probe_guest_count_candidates()` (`scraper/goccl.py`, via `glom`) safely tries several plausible-but-unconfirmed candidate paths on every live GoCCL check and surfaces (via a `WARNING`-level log + the full `initialData` blob, now captured in the raw dump) whichever ones resolve to something — purely diagnostic, never fed into `guests_count`/`guests_count_verified`. The actual guess-count behavior (a global default, explicitly marked unverified in the result note) is unchanged; this only makes it possible to close the gap for real next time a live GoCCL booking is checked, instead of guessing now.

### 12. NCL's entire comparison model was wrong, plus five more bugs found only by running it live

2026-08-26, the day NCL went from never-executed to verified working. Full detail in [Norwegian Cruise Line (NCL) Reference](#norwegian-cruise-line-ncl-reference); recorded here because each one is a distinct, generalizable lesson:

1. **The core business question was wrong** — the scraper searched for a cheaper *different* category when NCL repricing is actually a **same-category** reprice. No amount of code review would have caught this; it took the project owner stating it directly plus a real recorded session to confirm. **Lesson: verify what the business question IS before hardening how it's answered.**
2. **Playwright auto-dismisses unhandled dialogs, and this portal raises three kinds** — the unlock-confirmation modal (so the 30-minute lock likely never actually released), the category-change `confirm()` (so every re-selection silently failed), and `beforeunload` on nearly every navigation during an edit (so navigations could silently not happen). **Lesson: on a dialog-heavy portal, install ONE persistent `page.on("dialog")` handler that branches on type and logs everything — per-action `once()` handlers leave every other moment uncovered, defaulting to dismiss.**
3. **SlickGrid is virtualized and its jQuery instance handle wasn't exposed** — only 23 of 40+ rows were in the DOM, the target category sat at index 26, and the scroll that should have fixed it silently never ran. **Lesson: never assume all rows are in the DOM, and never depend on a framework's internal handle being reachable — scroll the viewport element directly.**
4. **Restored cookies broke login via CSRF mismatch** — `BaseScraper` restores `storage_state` every launch; submitting a SilverStripe login form against stale cookies is silently rejected with *no error text at all*. **Lesson: check whether an existing session is still valid before re-authenticating, and clear stale cookies before submitting a CSRF-protected form.**
5. **`navigate()` had no retry anywhere in the project** — a real gap given ESPRESSO's own documented history of isolated `Failed to fetch`/`ERR_ABORTED` blips cascading into the *next* booking also failing. Now retries (3 attempts, 2s × 1.5 backoff) while re-raising dead-browser errors immediately. **Deliberately scoped to `navigate` only — the mutating confirm/cancel-edit clicks must never be retried, since a retried submit could double-submit against a real booking.**
6. **The addon table was parsed positionally against a 3-column layout** (Guest Name | Addon Name | Quantity), so guest names were being read as addon names on every row. Now located by header text.

**Two self-inflicted bugs caught by local self-testing before any live run**, worth recording because both were invisible to inspection: passing `action=` to `log_action()` collided with that function's own first parameter and raised only as a stray asyncio traceback inside an event handler; and installing the dialog handler as step 0 made it the first code to touch `self.page`, whose property raises a plain `RuntimeError` — not dead-browser-shaped — which got swallowed into an ordinary ERROR result and defeated `BookingService`'s restart path (caught by an existing regression test).

### 13. A 1-character password silently broke every NCL login — `getpass` was dropping pasted text

Real incident, 2026-08-26. NCL logins kept failing with no clear cause. The credential stored in the OS credential store turned out to be **one character long**: `getpass.getpass()` reads the terminal with echo disabled, and in this Windows terminal host a Ctrl+V paste was not delivered as keystrokes to that raw read — so the pasted password was silently dropped while the blank prompt gave no feedback either way. This was especially costly because **NCL shows no error text on a rejected login**, so a bad credential looked like a scraper timeout bug rather than a credential problem.

Fixes in `save_login.py`: a `msvcrt.getwch()`-based console reader (reads the console *input buffer*, which is where a paste actually lands) that echoes `*` plus a **live character count** so a failed paste is immediately visible; a Tk GUI dialog fallback; `getpass` demoted to last resort; and a length guard that refuses an implausibly short password. Two follow-on bugs found while building it: `sys.stdin.isatty()` wrongly returned `True` on a pipe and **hung a real command** (now uses `ctypes.GetConsoleMode`, which correctly distinguishes a console from a pipe), and the length guard originally left a previously-saved bad password in place when declined (now clears it, so the state is unambiguous — either a good credential is stored or none is). **Lesson: a hidden input with no feedback is a silent-corruption vector; always echo *something* (even just a count).**
...
1. ~~`BookingService.has_live_session()` does not verify the underlying browser is actually connected — a pure in-memory check that would still report `True` after the browser crashed.~~ **Resolved, doc was stale (found and fixed during a 2026-08-25 audit)**: `has_live_session` (`services/booking_service.py`) now additionally checks `self._live_scraper.is_alive`, so a crashed/disconnected browser is correctly treated as no live session, not silently accepted as one.

2. ~~The CSV export is missing the `Lost Fares` / `Re-addable Fares` / `Gained Fares` columns that the Excel export already includes.~~ **Resolved, doc was stale (found and fixed during a 2026-08-25 audit)**: `services/csv_export.py` now writes `result.lost_fares`, `result.re_addable_fares`, and `result.gained_fares` alongside `lost_pkg_names` — CSV and Excel exports are at parity.
...
5. ~~`easy_menu.py`'s `menu_scan()`/`menu_watch()` build a `SimpleNamespace` missing the `capture_market_data`/`capture_everything` attributes.~~ **Resolved, doc was stale (found and fixed during a 2026-08-25 audit)**: both call sites now explicitly set `capture_market_data=False, capture_everything=False` on the constructed `SimpleNamespace` — the latent `AttributeError` risk is closed.

6. **This document predates the GoCCL scraper/adapter and does not cover it in depth.** GoCCL Navigator is Carnival Cruise Line's travel-agent portal (`scraper/goccl.py`'s `GoCCLScraper`, `core/calculator.py`'s `calculate_goccl`) — structurally closer to ESPRESSO/NCL than to MSC (same category-upgrade-search shape), but GoCCL can't confirm per-fare-code OBC without clicking through, so `calculate_goccl` only ever returns UNCONFIRMED candidates, never a fully confirmed OPTIMIZATION the way ESPRESSO's `_confirm_candidate_total` does. Treat `scraper/goccl.py`, `core/calculator.py`'s `calculate_goccl`, `extension/adapter_goccl.js`, and the GUI/CLI code itself as authoritative over this document for anything GoCCL-related until this doc gets a full reference section for that cruise line.

7. ~~Paid-in-Full tolerance rule (1.5% of total, min $25) may be stricter than what "paid in full" means in practice — booking 3000001 ($370.84 due on $8,892.68, 4.17%) was flagged as one that should have been caught and wasn't.~~ **Resolved 2026-08-04, doc was stale**: `PAID_IN_FULL_TOLERANCE_PCT` (`core/calculator.py`) was widened from 1.5% to **5%**, specifically clearing 3000001's 4.17% with a little headroom — chosen as "the smallest round number that clears it," a business-risk-tolerance choice, not a derived fact (it also newly classifies ~8% more of all captured bookings as paid-in-full). If 5% turns out to be too aggressive or not aggressive enough in practice, this one constant is the only thing that needs to change.
...
9. **`run_persistent_watchlist_scan.py` does a full re-scan on restart, not a true resume — investigated 2026-08-13, deliberately not implemented.** A hard process kill mid-scan loses all progress: `last_hash = None` at startup means the very next loop iteration treats the current watchlist as "changed" and rescans the entire deduped list from index 0, and `ScanJobRecord.progress_done` is only ever written once, in `_run_batch`'s `finally` block at the very *end* of a batch — a killed process leaves no usable mid-scan checkpoint even for a future resume-aware implementation to read. No evidence found anywhere in this project's history that "resume after crash" was ever an explicit requirement; the real existing mitigation against redundant work is the 12h NO_SAVING TTL cache, and watch-runs already pass `bypass_cache=True` (this script exists specifically to re-check the same bookings repeatedly). Not implemented because a safe checkpoint mechanism needs to distinguish "this booking's result was durably persisted before the crash" from "this booking was in-flight, outcome unknown" — getting that wrong risks silently skipping a real booking or double-processing one, which is worse than the current simple (if wasteful) full-rescan behavior. *(Researched 2026-08-25: `persist-queue`'s `SQLiteAckQueue` is a well-matched off-the-shelf fit for exactly this ack/requeue semantic if this decision is ever revisited — lower-risk than a bespoke checkpoint since the durability logic is already tested upstream.)*

10. **The Chrome extension's NCL path is known-incorrect and diverges from the Python platform.** Confirmed 2026-08-26: `background.js`'s `handleNCLBooking` / `adapter_ncl.js` still implement the switch-to-a-different-cheaper-category model that was **proven wrong** against the real portal (NCL repricing is a same-category reprice), and the extension also lacks the three dialog fixes, the SlickGrid-virtualization fix, and the protected-promo (LATRIPLE) gate. This breaks the monorepo's usual "either implementation produces the same verdict" guarantee for NCL specifically. **Treat extension NCL results as unreliable.** Bringing the extension in line is real, un-started work — the fixes are non-trivial (the dialog handling in particular has no direct MV3 equivalent to Playwright's `page.on("dialog")`, since the extension can't intercept a native `confirm()` the same way).

11. ~~`FREESRVC` may deserve the same never-lose protection as `LATRIPLE`.~~ **RESOLVED 2026-08-26** — the project owner confirmed it is the same case, and `FREESRVC` is now in `NCL_NEVER_LOSE_PROMOS` alongside `LATRIPLE`. See the [NCL Reference](#norwegian-cruise-line-ncl-reference) for the live-verified effect (two of the four known bookings correctly flipped from `OPTIMIZATION` to `TRAP`). **Still worth watching:** other codes in that promo family may deserve the same treatment — `INTSHO`/`DISXIN` (Free-at-Sea perk-selection markers), `LATITUDE` (member discount) and `MILITARY` (military rate) all represent real value that a reprice could plausibly drop, but none has been observed being lost in a real capture yet, so none has been added on speculation.

12. **`calculate_ncl`'s `lost_fobc` check does not match the real promo codes** (`FREESRVC`→`FITOBC`, neither containing the literal `"FOBC"` it tests for), and the observed direction is opposite to what the code assumes — see the defect note under [`calculate_ncl` classification](#business-logic-reference). Not patched, because a string-only fix would be wrong; the underlying question of whether an addon *swap* should reduce `net_saving` at all is unresolved.
...
### Saving/clearing a portal login (`save_login.py`/`clear_login.py`)

```
python save_login.py     # or double-click SAVE_LOGIN.bat (Windows) / SAVE_LOGIN.command (macOS)
python clear_login.py    # or CLEAR_LOGIN.bat / CLEAR_LOGIN.command
```
Menu-driven — pick MSC or ESPRESSO/Royal, then enter the username and password (hidden, pasting is fine). Stored encrypted via `keyring` in the OS's own credential store (Windows Credential Manager / macOS Keychain), tied to the local device only — never written to a file, never typed into chat, never exportable to another machine. See [MSC Cruises Reference](#msc-cruises-reference) for how MSC's saved credential is consumed by `auto_login`; ESPRESSO's saved credential only pre-fills the login form today (MFA is still manual, and autofill-on-the-real-page is not yet wired up).

### Analyzing accumulated scan history (`analyze_history.py`)

```
python analyze_history.py [--save report.txt]
```
Read-only report over the existing `cruise_intel.db` and MSC's `data/msc_control/*.jsonl` — status/opportunity breakdowns, savings totals, hit rates by cruise line and price category, and (via `mlxtend`) frequent-pattern mining looking for above-baseline combinations worth a closer look. Does not collect any new data or change any stored result.

### Onboarding a new, unfamiliar cruise-line portal (`discover_portal_fields.py`)

```
python discover_portal_fields.py <url>
```
Opens a real, visible browser; you navigate and log in yourself (it never fills in or clicks anything). Press Enter when looking at a page you want help with, pick what you're hunting for (booking search box, login fields, submit button, or a custom role+keyword query), and it prints a ranked list of candidate elements with a suggested selector — a heuristic guess from visible attributes, always to be verified against the real page before it goes into an actual scraper. See [MSC Cruises Reference](#msc-cruises-reference) for why this stays classical/heuristic rather than AI-based.

### NCL live check (`run_ncl_live_check.py`)

```
python run_ncl_live_check.py 3000007 --dry-run                      # safe: never enters edit mode
python run_ncl_live_check.py 3000007,3000006,3000004,3000003     # full check, all four
```
Always non-headless (no override — watch it). Logs in automatically from the saved credential when available, falling back to a manual login. `--dry-run` validates login, search, the `__preloaded_data` read, and addon parsing, then stops **before** entering edit mode — do this first. Note `--dry-run` cannot produce the "Current Price" column, because reading the live category price requires the Category tab, which is only reachable after entering edit mode. The full run prints every dialog the portal raised, a summary table in the same shape as the project owner's own report, and saves a Playwright trace (`playwright show-trace data/ncl_live_test/trace.zip`) plus the standard raw capture. See [Norwegian Cruise Line (NCL) Reference](#norwegian-cruise-line-ncl-reference).

### Diagnosing a failed live run from the trace (`BaseScraper.trace_path`)

Any scraper can now record a full Playwright trace by setting `scraper.trace_path` before `start()` — Playwright's own built-in tracing (`screenshots=True, snapshots=True, sources=True`), so no new dependency. Captures a replayable DOM/network/console/screenshot timeline of the entire session, saved before the context closes and in its own try/except so a tracing failure can never skip browser cleanup. **The point is diagnosing a failure without needing a second live run against a real portal** — which matters most for NCL, where every attempt risks a 30-minute booking lock. Off by default (`None`); zero behavior change for callers that don't set it. View with `playwright show-trace <path>`.
...
- **Senior discount**: 5% for Inside/Outside/Ocean View, **10% for Balcony/Suite/Yacht Club** (confirmed via real SRN math: `$79.20 / $88.00 = 0.90` exactly on a Balcony booking) — requires **at least two** senior (65+) passengers in the cabin, not just one. **CORRECTED 2026-08-18** (real incident, booking 3000030: a single senior passenger showed green/eligible in the GUI): the original `all_seniors` flag ("every passenger in the cabin is 65+") was never the real eligibility test — it wrongly *excluded* a valid 2-seniors-plus-grandchildren cabin (not every passenger senior) while wrongly *including* a lone senior traveling alone (trivially "all 1 passengers are seniors"). `_filter_out_ineligible_senior_discount(options, senior_count)` (`core/calculator_msc.py`) now strips SENIOR DISCOUNT out of the addable-options list unless `senior_count >= 2`; non-senior passengers alongside 2+ seniors (e.g. grandchildren) do not disqualify it — only the raw senior count matters. Never gets its own disclosure line in the itemized Price Breakdown (see `isInv` above); detected instead by comparing the SRN line against the standard undiscounted NCF-by-length table (7 nights = $182.00, 4 nights = $88.00, 3 nights = $66.00 — non-linear, not a flat per-diem rate).
- **The senior-discount blind spot, and why a clean SRN match isn't proof either way** (real false positives caught 2026-08-24, bookings 3000018/3000017): because senior discount never prints its own line item, the SRN-vs-standard-table comparison above only works when the booking's sailing length has a known entry in the standard NCF table (`STANDARD_NCF_BY_NIGHTS`) — `_srn_reference_available(summary_text)` (`msc_commands.py`) checks this. When the sailing length ISN'T in that table, the math can neither confirm nor rule out a silent senior discount already being applied, so a plain "DISCOUNT_ADD: SENIOR DISCOUNT available" finding would be a false positive on a booking that already has it. `evaluate_msc_booking` now takes `senior_discount_verifiable: bool` (from `_srn_reference_available`); when `False`, any SENIOR DISCOUNT option is downgraded out of the confident `OPPORTUNITY` list into its own `INSUFFICIENT_DATA` bucket (`_check_discount_add`, `core/calculator_msc.py`) rather than reported as a clean opportunity — other addable discounts (e.g. Voyagers Club) are unaffected and still report normally.
- **Price-match is time-gated by the final payment date, not just paid-in-full status** (real false positive caught 2026-08-24, booking 3000031: final payment date 08/20/2026 already passed, so a PRICE_MATCH finding was wrong even though the booking wasn't paid in full). MSC (matching general cruise-industry practice) only informally honors a fare drop before final payment is due — once that date passes the fare is locked in. `_final_payment_date_passed(text)` (`msc_commands.py`) regexes the `Final Payment Date` line and parses it via `dateparser.parse(..., settings={"DATE_ORDER": "MDY"})`, comparing against `datetime.now()`. `_check_price_match` (`core/calculator_msc.py`) checks this gate right after (and structurally the same shape as) the `is_paid_in_full` gate — a booking can be past its final payment date without having paid yet, so the two checks are independent. Scoped to `PRICE_MATCH` only, same as `is_paid_in_full`: `DISCOUNT_ADD`/`DISCOUNT_TIER_UPGRADE`/`VOYAGERS_SELECTION` are unaffected.
- **A discount-add finding for Voyagers Club must say whether the customer is actually a member** (real false positive caught 2026-08-24, booking 3000019): MSC's Additional Discounts dropdown offers "Voyagers Club 5%" as addable regardless of whether the customer has ever enrolled, so a plain "DISCOUNT_ADD available" finding reads as a clean opportunity when it may not be one — enrolling first is an extra step, not something a phone call alone can apply. Researched directly (2026-08-24): plain Voyagers Club enrollment is free and instant, needs only the booking reference — separate and faster than a *Status Match* (also free, ~72hr approval, unlocks up to Diamond tier via 55+ partner loyalty programs), which is the slower path Neon's "not worth it" framing may have assumed. `evaluate_msc_booking` now takes `has_voyagers: bool`; when `False`, `_voyagers_club_addable_label` (`core/calculator_msc.py`) annotates the Voyagers Club option with this distinction directly in the finding's label instead of presenting it as an unconditional win.
...
### Rate-tab matching: `_select_matching_tab`'s six tiers
...
6. **RapidFuzz fuzzy fallback** — added 2026-08-24 (`fuzz.token_set_ratio`, classical text-similarity, not AI — see the project's [[cruiseintel-no-ai-dependencies]] rule) for cases tiers 1-5 all miss, most commonly a plain typo (real catch: `"SALE DRINK AND WIFI"`, singular typo, vs. `"FLASH SALE DRINKS AND WIFI"` — tiers 3/4 both fail on the missing "S", tier 6 scores it 84/100 with a 48-point gap to the next candidate). Only runs when tiers 1-5 found nothing AND none of them already set an `ambiguous_note` (an explicit "these two are too close to call" from an earlier tier is never overridden by a fuzzy guess). Scores every remaining candidate tab against the rate name and requires **both** a floor (`_FUZZY_SCORE_FLOOR = 80`) and a gap to the second-best match (`_FUZZY_GAP_FLOOR = 20`) before accepting a match; a high score with no clear gap sets its own `ambiguous_note` instead of guessing. Validated against all 12 pre-existing tier 1-5 test cases before shipping (zero behavior change to any of them) plus the new typo-catch case above.
...
### Robust text extraction: `price-parser`/`dateparser` in `msc_commands.py`

Added 2026-08-24: `_extract_booking_essentials` now captures whole lines (`"Booking Value\n(.+)"`, `"Due Amount\n(.+)"`) and hands them to `price_parser.Price.fromstring()` instead of a narrower hand-rolled decimal regex, and a new `_final_payment_date_passed(text)` helper regexes the `Final Payment Date` line and parses it via `dateparser.parse(..., settings={"DATE_ORDER": "MDY"})`, compared against `datetime.now()` — this backs the price-match final-payment-date gate documented above. The Overpayment extraction was fixed after a self-caught regression: an early version (`Overpayment[^\n]*`) stopped at the newline, but the real page shape (confirmed on booking 3000021: `"Overpayment\n$0.02"`) puts the amount on the *next* line — fixed to `Overpayment[^\d\-]{0,20}(-?\$?[\d,]+(?:\.\d+)?)`, whose `[^\d\-]` skip-pattern deliberately crosses the newline before `Price.fromstring()` parses the captured amount.

**This same hardening was deliberately NOT extended to ESPRESSO's `_price_from_row`** (`core/calculator.py`, backs `find_upgrade_candidates`) — attempted, then reverted, 2026-08-24. `Price.fromstring()` run against ESPRESSO's full multi-field `rowText` blob grabbed OTHER embedded bare numbers instead of the real price on real captured rows (confirmed against all 83,296 captured category rows: a genuine $8,782.00 Royal Suite row parsed as $0.00, because the row also contains fragments like `"WLT(0)"`/`"AVL(2)"`). The original exact-decimal-shape regex (`_ROW_PRICE_RE = re.compile(r"([\d,]+\.\d{2})")`) is actually the correct disambiguator for this specific case, unlike MSC's/GoCCL's, where the fix target was already isolated, single-purpose text before parsing — a real example of why every "harden this" pass in this project gets verified against real captured data before shipping, not applied uniformly on the assumption that a better library is automatically a better fix.

### ARIA structure-drift monitoring: `BaseScraper.check_structure_drift`

Added 2026-08-24, applies to all scrapers (not MSC-specific, lives in `scraper/base.py`). Playwright's built-in `locator.aria_snapshot()` (zero new dependency) captures a structural snapshot of a selector's accessibility tree; `check_structure_drift(name, selector)` compares today's snapshot against a saved baseline file (`data/structure_baselines/<name>.yaml`), creating the baseline on first run and returning `"changed"` (logged as a warning, plus `log_action("structure_changed", ...)`) whenever the live structure no longer matches — an early-warning signal that a portal's DOM has shifted in a way that could silently break a hand-written selector, without waiting for an actual scrape failure to notice. Gated to run at most once per scraper session per `name` (`self._structure_checked: set[str]`) so it doesn't re-check on every retry within the same run. Wired into `scraper/espresso.py`'s `check_booking` right before `_search_booking`, watching both the search input and search button selectors. The real ESPRESSO baseline files don't exist yet as of this writing — they're created automatically the first time this runs against a live session.

### Data-mining the accumulated scan history: `analyze_history.py`

Added 2026-08-24, root of `platform/`, read-only against the existing SQLite `cruise_intel.db` and MSC's `data/msc_control/*.jsonl` — no new data collection, just analysis of what's already been captured. `espresso_section`/`msc_section` report status/opportunity breakdowns, savings totals, per-cruise-line and per-price-category hit rates, the c3-caveat frequency, error-day clustering, and (MSC-specific) the senior-discount-blind-spot exposure rate and discount-option frequency. `espresso_pattern_mining_section` runs `mlxtend`'s `apriori`/`association_rules` (classical frequent-itemset mining, not machine learning) over a one-hot-encoded frame of `cruise_line`/`price_category`/`has_c3_caveat`/`OPTIMIZATION`, filtered to rules whose consequent is `OPTIMIZATION`, deduped by result-row-set (keeping the simplest antecedent when two rules cover identical rows), with raw occurrence counts shown alongside percentages and a "small sample" flag under 10 occurrences. Real finding surfaced this way: Category V2 bookings carrying a c3 caveat hit `OPTIMIZATION` 21.4% of the time (3/14) vs. V2's own 4.6% baseline — a genuine, above-baseline pattern, not yet acted on. Run with `python analyze_history.py [--save PATH]`.

### Heuristic new-portal onboarding: `scraper/smart_locator.py`/`discover_portal_fields.py`

Added 2026-08-25, in direct response to a hard project rule (see below): **no AI, no LLM, no API key, anywhere in this project — not even for one-time, human-supervised onboarding tooling.** `scraper/smart_locator.py`'s `find_candidate_fields(page, keywords, role)` enumerates every element of a given ARIA role on the current page and scores its visible contextual attributes (id, name, placeholder, aria-label, title, data-qa, associated `<label>` text) against the given keywords using RapidFuzz (`fuzz.WRatio`) — the same category of plain heuristic a password manager's autofill already uses to guess "this is the username field," not a model reading and understanding the page. Validated (`tests/test_smart_locator.py`) against REAL, already-confirmed field attributes from this project's own scrapers — ESPRESSO's `#reservationid`/`data-qa="secure.espresso.input.reservation.search"` search box, MSC's `#club-firstname` Voyagers-insert field — ranking correctly above plausible decoys before ever being trusted on an unfamiliar page. `discover_portal_fields.py` wraps this in an interactive CLI: it opens a real, visible browser at a given URL, a human navigates and logs in manually (the script never fills or clicks anything itself), and on request it ranks candidates for common categories (booking search box, username, password, submit button) or a custom role+keyword query, printing a suggested selector for each — always to be verified against the real page before it becomes actual `BaseScraper` code, the same discipline `CONTRIBUTING.md` already establishes for `playwright codegen` output. Intended strictly for drafting a new cruise line's scraper faster; never used in the live, recurring scan pipeline.

**Project-wide rule: no AI/LLM/API keys, anywhere, not even for one-time onboarding tooling.** Stated explicitly and emphatically by the project owner, 2026-08-25, after an LLM-based browser agent (scoped only to one-time, human-supervised new-portal onboarding, never touching live scans) was offered as an option and rejected outright: "I AM NOT LOOKING TO BUILD ANYTHING WITH AI OR API KEYS I JUST WANT A FUNCTIONING PROGRAM THAT IS AS SMART AS AN AI LEVEL." This is a blanket rule, not scoped to risk level or use case. Every task in this project that would normally be a good fit for an LLM (fuzzy/semantic text understanding, "explore this unfamiliar page and describe it," anomaly explanation) gets a classical/deterministic technique instead — regex + a real parsing library, RapidFuzz, `mlxtend`, ARIA-snapshot/DOM heuristic scoring, statistics — never an LLM call. `smart_locator.py` above, the RapidFuzz rate-tab-matching tier, `mlxtend` pattern mining, and ARIA structure-drift monitoring are the working examples of this in practice.

### Credential storage tools: `save_login.py`/`clear_login.py`

Added 2026-08-24/25 (generalizing the MSC-only `msc_save_credentials.py`/`msc_clear_credentials.py`, which still exist and still work, to also cover ESPRESSO/Royal). Menu-driven scripts using `keyring` (Windows Credential Manager / macOS Keychain / Linux Secret Service, whichever is native) with `getpass()` so nothing is echoed, logged, or written to any file — storage is encrypted by the OS itself and tied to the local device, with no exportable file this produces (cannot be copied to a server, USB drive, or another machine and read there). Strips bracketed-paste escape sequences (`_BRACKETED_PASTE_RE = re.compile(r"\x1b\[20[01]~")`) before saving, since some terminals wrap pasted text in invisible markers that would otherwise corrupt a pasted password. `SAVE_LOGIN.bat`/`CLEAR_LOGIN.bat` (Windows, using the in-repo `venv\Scripts\python.exe`) and `SAVE_LOGIN.command`/`CLEAR_LOGIN.command` (macOS, bootstraps its own `~/.cruiseintel_login_venv` on first run) let the project owner double-click rather than use a terminal directly. ESPRESSO requires MFA at login, so saving its credentials here only pre-fills the username/password fields — it does not make ESPRESSO's login fully unattended the way MSC's `auto_login` is; the MFA step is still manual every time. Actually wiring the saved ESPRESSO credential into an autofill on the real login page is separate, not-yet-built work — these scripts only store the credential for when that lands.

### MSC in the desktop GUI: `services/msc_live_service.py`

Added 2026-08-24. `MscLiveService` drives MSC in-process for the PySide6 GUI (`gui/windows.py`) — `ensure_started()`/`stop()`/`check_login()`/`run_batch()`, plus `is_alive`/`is_running` properties — as a GUI-facing counterpart to the existing command-file-driven `msc_commands.py`/`msc_session_controller.py` console workflow (both exist; the GUI does not replace the console controller). `gui/windows.py` re-added MSC to its cruise-line dropdown, branches `_on_login_check`/`_on_start` per cruise line, and added `_run_msc_batch`/`_append_msc_result_row`, extending `_color_for_status` and `_on_export` to handle MSC's own result shape (`MscBookingResult`) alongside the existing `BookingResult`.

**Real incident, 2026-08-24: a long-running GUI process silently executed stale code after a fix shipped.** Booking 3000030 kept showing a 6-day-old, incorrect green (opportunity) result in the GUI even after the senior-discount fix (above) had already shipped, and a live 26-booking batch run through the GUI failed on every single booking with `TypeError: evaluate_msc_booking() got an unexpected keyword argument 'all_seniors'` — a signature that had already been renamed to `senior_count` in the current `msc_commands.py`. Root cause: the GUI process (confirmed via `Get-CimInstance Win32_Process` process-start-time inspection) had been running, unchanged, since before that fix was written, and Python had already imported `msc_commands` into memory — editing the file on disk doesn't change what a long-running process is executing. The console-based `msc_session_controller.py` already re-imported the module fresh per command and never had this problem; `services/msc_live_service.py` did not, because it holds the process open across an entire GUI session. Fixed by adding `importlib.reload(msc_commands)` at the top of both `check_login()` (before `auto_login()`) and inside `run_batch()`'s loop (before each `_check_booking_msc()` call) — the GUI now always runs the current on-disk code, matching the console controller's existing behavior. **Operational lesson: after any `msc_commands.py` change, a GUI session left open from before the change needs a restart of the automation (or, now, will self-heal on its next login-check/batch call) — a stale-looking green result is a real signal worth checking against the actual code version, not assumed to be a fluke.**

---

## Norwegian Cruise Line (NCL) Reference

Added 2026-08-26, when NCL went from never-having-been-run to verified working end-to-end. **Everything in this section is confirmed against the real live `seawebagents.ncl.com` portal**, not inferred — before this date, zero NCL bookings had ever been recorded anywhere in this project's data (verified by direct DB query and a content grep across `data/`).

### CRITICAL CORRECTION: NCL is a SAME-CATEGORY reprice, not a category switch

**The original NCL design was wrong about the core business question.** `scraper/ncl.py` (ported from `adapter_ncl.js`) implemented: read all categories, filter to those cheaper than the current one, sort descending by `resTotal`, pick the smallest-drop candidate, and switch the booking into that DIFFERENT category. Confirmed directly by the project owner 2026-08-26 and independently by a real recorded portal session: **that is not what NCL repricing is.** The real opportunity is "has *this same category's* price dropped since the booking was made" — the same shape as ESPRESSO's reprice check, not a GoCCL/upgrade-style cross-category comparison.

The corrected flow in `check_booking`: search → read `__preloaded_data` → paid-in-full short-circuit → scrape addons → enter edit mode → Category tab → **read the booking's OWN category row's live `resTotal` from the grid** (no click needed for this part — the live price is already there) → if it differs from the booking's locked-in total, **re-select the SAME category** → read the recalculated promo/addon state → calculate → always cancel edit. If the live price matches the locked-in total, it returns early without the invasive re-selection at all.

**⚠ PARITY DIVERGENCE, currently unresolved:** the Chrome extension's `handleNCLBooking` (see [adapter_ncl.js](#adapter_ncljs) above) still implements the OLD switch-to-a-cheaper-category model. The Python platform and the extension therefore no longer agree on NCL, breaking this monorepo's usual "both implementations produce the same verdict" guarantee. The extension's NCL path should be treated as **known-incorrect** until it's brought in line.

### Verified live results

All four of the project owner's own independently-produced expected rows reproduced exactly, and stably across repeated runs:

| Reservation | Category | Invoice | Current | Difference |
|---|---|---|---|---|
| 3000007 | BA | $2,878.00 | $2,858.00 | $20.00 |
| 3000006 | MB | $828.00 | $768.00 | $60.00 |
| 3000004 | IA | $3,388.00 | $3,348.00 | $40.00 |
| 3000003 | IT | $2,153.10 | $2,081.10 | $72.00 |

### THREE dialog bugs — this portal is unusually dialog-heavy, and Playwright's default is to DISMISS

The single most important NCL lesson. Playwright auto-**dismisses** any unhandled `dialog` event, which is the wrong answer for every dialog this portal raises. All three are now handled by ONE persistent `page.on("dialog", ...)` handler (`NclScraper._install_dialog_handler`), installed as step 0 of `check_booking` before any navigation, which branches on `dialog.type` and records everything to `scraper.dialogs_seen`:

1. **`#dialog_confirm_ok` — the unlock confirmation.** Clicking `#res-edit-cancel` ("CANCEL EDIT") does **not** by itself release the 30-minute lock: the portal then shows a modal ("Current Reservation will be deleted from temporary storage!") whose OK button must also be clicked. Before this fix the lock very likely never actually released on any real run — the worst failure mode this whole scraper guards against.
2. **The category-change `confirm()`** ("Selecting new category may result in change of fares. Do you want to proceed?"). Auto-dismissed = equivalent to clicking Cancel, so the selection is rejected server-side while the click still "succeeds" from Playwright's perspective. Every real re-selection would have silently failed.
3. **`beforeunload` dialogs.** This portal raises them constantly during an active edit (24 in one recorded session). Dismissing a `beforeunload` means "stay on this page" — i.e. it **cancels the navigation**, leaving the scraper reading a stale page and drawing conclusions from it. Accepted explicitly.

Anything genuinely unrecognized (`alert`/`prompt`) is **dismissed and logged loudly** — never blind-accepted on a real client booking.

### SlickGrid is VIRTUALIZED — the jQuery grid handle is not exposed

**Confirmed real bug, booking 3000003 / category IT.** SlickGrid only renders the rows currently in view. That booking's grid had 40+ categories but **only 23 in the DOM**, and `IT` sat at **grid index 26** — outside the rendered window — so a DOM row search found nothing and the check failed with "Category re-selection failed" even though IT's correct price ($2,081.10) was in the data model the whole time.

The original scroll attempt used `$(gridEl).data('SlickGrid').scrollRowIntoView(idx)`. Live diagnostics proved **that handle is not exposed on this portal** (`gridInstanceReachable: false`), so the scroll silently never ran. The fix scrolls the `.slick-viewport` element **directly** (needs no grid instance): estimate the offset from a rendered row's height, jump there, then sweep top-to-bottom as a fallback — re-querying after every scroll, since virtualized rows are destroyed and recreated as they move through the viewport. **Generalizable lesson for any future grid-based portal: never assume all rows are in the DOM, and never depend on a framework's internal instance handle being reachable.**

Two related fixes in the same function: the `HasAvailability` requirement was **removed** (a leftover from the switch-category design — for a same-category reprice the guest already occupies the category, so "open to new bookings" is a different question and must not block repricing what they already hold), and `_select_category` now returns a **structured reason** (`category_not_in_grid_data` / `row_never_rendered_after_scrolling` / `select_link_not_found_in_row` / an exception message) instead of a bare `False`. The bare boolean is precisely why the IT failure needed a dedicated live diagnostic run to identify.

### Login: `auto_login`, and the stale-cookie vs. CSRF trap

`NclScraper.auto_login()` mirrors MSC's `auto_login` contract exactly — reads credentials from the OS store via `keyring` under `settings.ncl_credential_service`, and **never raises**, returning a status string (`OK` / `NO_CREDENTIALS_SAVED` / `INVALID_CREDENTIALS` / `TIMEOUT_WAITING_FOR_LOGIN` / `ERROR: ...`) so a failed auto-login falls back to a manual login instead of crashing a run. Confirmed real selectors: `#LoginForm_LoginForm_Email` (labeled "Username" on screen — the id says Email but the real value is a plain username), `#LoginForm_LoginForm_Password`, `#LoginForm_LoginForm_action_doLogin`. Login URL is its own setting (`settings.ncl_login_url`, `/Security/login/`), distinct from the search URL.

**Unlike ESPRESSO, NCL's login is genuinely automatable** — no MFA, no SSO redirect, no cookie banner observed. This makes NCL the second cruise line after MSC where saved credentials produce a fully unattended login. (A newer NCL SSO layer branded "Norwegian Central" exists; it was confirmed **not** to apply to this account — the URL stayed on `seawebagents.ncl.com/Security/login/` throughout.)

**CONFIRMED REAL TRAP:** `BaseScraper.start()` restores `storage_state` on every launch, and SeaWeb is SilverStripe, which embeds a per-session `SecurityID` CSRF token in the login form. Submitting that form while **stale restored cookies** are present is silently rejected — the page just re-renders the bare form with a `#LoginForm_LoginForm` URL fragment and **no error text whatsoever**. Two fixes: (1) `_check_login()` runs first, and if the restored session is still valid the login form is never touched (also correct because NCL, like ESPRESSO, appears to allow one active session per account); (2) if not authenticated, `context.clear_cookies()` is called before submitting, so the form token and session cookie come from the same fresh session. Because NCL shows no error text on rejection, `auto_login` also now detects "password field still present after submit" and reports `INVALID_CREDENTIALS` rather than the misleading `TIMEOUT_WAITING_FOR_LOGIN`.

### HARD RULE: never reprice away a protected promo (LATRIPLE)

Stated directly by the project owner 2026-08-26. **A booking must never be recommended for repricing when `LATRIPLE` is present BEFORE and absent AFTER** — i.e. when the reprice would lose it. All other combinations are allowed (kept, gained, or never present).

**What LATRIPLE is** (researched 2026-08-26; moderate-high confidence — it's an internal B2B promo marker with no official NCL documentation, so this rests on corroborating travel-agent/cruiser sources rather than a primary one): **triple Latitudes points** (Latitudes Rewards is NCL's loyalty program), applying to guests 1–2. `LATREW` — exactly what replaced it on real booking 3000003 — is the **baseline** marker for standard 1-point-per-night accrual, so a `LATRIPLE`→`LATREW` swap is a silent **3x → 1x loyalty-points downgrade**. That justifies a hard block rather than a dollar subtraction: there is no invoice line item to net off, the loss is irreversible once repriced, and Latitudes tiers gate real recurring benefits (free bags, priority, dining/excursion credits).

Implemented in `core/calculator.py` as `NCL_NEVER_LOSE_PROMOS` (a frozenset, so more codes can be added), `_split_promo_codes()` (handles **both** real formats — the booking header is comma-separated, the category-grid row is pipe-separated), and `ncl_lost_protected_promos()`. Wired into `calculate_ncl` as a hard gate checked **before any status is assigned**, so it can never surface as `OPTIMIZATION` regardless of the fare drop; produces `TRAP` with an explicit note and confidence 1. Verified live: 3000003 returns TRAP ("would LOSE LATRIPLE … for only $72") while the other three stay OPTIMIZATION.

**`FREESRVC` is protected too — added 2026-08-26 on the project owner's explicit instruction** ("this is the same case like latriple, FREESRVC add rule"), and it is in fact **better documented than LATRIPLE**: a real NCL travel-agent promo flyer confirms `FREESRVC` = "Free Pre-Paid Service Charges" (guests 1–2, Balcony and above, new FIT bookings only; excludes BX/MX guarantee categories and Studio/Inside/Oceanview), worth roughly **$20–25 per person per night** — plausibly $300–500 on a real sailing, an order of magnitude more than the fare drops being traded for it.

Confirmed real behavior, both live-verified: bookings **3000007** and **3000006** each had the reprice replace "FREE PREPAID SERVICE CHARGES" with a "Free $50 / $37.50 On-Board Credit Certificate" while the fare dropped only **$20** and **$60** respectively. Both were reported as `OPTIMIZATION` before this rule and are now correctly `TRAP`.

**Live-verified effect of both protected promos across the four known bookings** (this is the current, correct behavior):

| Reservation | Fare drop | Verdict | Protected promo lost |
|---|---|---|---|
| 3000007 | $20.00 | **TRAP** | `FREESRVC` |
| 3000006 | $60.00 | **TRAP** | `FREESRVC` |
| 3000004 | $40.00 | OPTIMIZATION | — (never had either) |
| 3000003 | $72.00 | **TRAP** | `LATRIPLE` |

So of the four bookings that all looked like clean optimizations, only **one** survives both protected-promo gates. If a reprice would lose both promos at once, the note names both (the helper returns a sorted list, not just the first match).

Other NCL promo codes researched (record so they aren't re-guessed): `LATITUDE` = Latitudes-member fare/discount; `INTSHO` (Internet & Shore Excursions) and `DISXIN` (Dining + Excursions + Internet) are Free-at-Sea perk-selection markers; `MILITARY` = NCL's military rate; `EASYFARE` = a fare-type marker that replaced `PG6`, with no perk value of its own; `FITOBC` = likely FIT onboard credit (**inference only**). `DISC50` / `SHX50` / `MAP10OFF` / `NCLHBENE` **could not be confirmed — do not trust guesses about these**.

### Per-guest addon parsing

The real Addons table is three columns — **Guest Name | Addon Name | Quantity** — with one row per (guest, addon) pair. The original scraper read it **positionally** (`cells[1]` as name, `cells[2]` as qty), which against the real shape would have read the *guest name* as the addon name and the *addon name* as the quantity on every row. `_scrape_addons` now locates columns by header text and returns `{name, qty, guest}`. `_summarize_addon_change(before, after)` diffs the per-guest lists and surfaces a "lost X; gained Y" summary in the result note — **informational only, deliberately not folded into `net_saving`**, matching the project owner's own reference report, whose dollar verdict is always the raw price difference regardless of addon changes.
