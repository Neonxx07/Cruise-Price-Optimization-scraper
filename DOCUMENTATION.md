...
11. **`FREESRVC` may deserve the same never-lose protection as `LATRIPLE`, and is better documented.** See the [NCL Reference](#norwegian-cruise-line-ncl-reference)'s promo section: a real NCL agent promo flyer confirms it as "Free Pre-Paid Service Charges" worth roughly $20–25 per person per night, and two of the four bookings verified live on 2026-08-26 lose it while still being reported as `OPTIMIZATION`. Needs the project owner's explicit decision before changing, since it would alter real recommendations on real bookings.
...
**⚠ OPEN QUESTION — `FREESRVC` is a strong candidate for the same protection and is BETTER documented than LATRIPLE.** A real NCL travel-agent promo flyer confirms `FREESRVC` = "Free Pre-Paid Service Charges" (guests 1–2, Balcony and above), worth roughly **$20–25 per person per night** — plausibly $300–500 on a real sailing. **Two of the four bookings checked live (3000007, 3000006) LOSE it** on the reprice, gaining a $50 / $37.50 On-Board Credit Certificate instead, and are still reported as `OPTIMIZATION` at $20 and $60. If that trade is bad, those are false positives of exactly the shape LATRIPLE was added to prevent. Deliberately **not** added without the project owner's explicit decision, since it changes real recommendations on real bookings.
