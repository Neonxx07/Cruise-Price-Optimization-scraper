...
**7. HARD BUSINESS RULE — never reprice away a protected promo.** A booking must NEVER be recommended when a protected promo code is present in the BEFORE promos and absent in the AFTER promos. **Two codes are protected**, both confirmed against real bookings:
   - **`LATRIPLE`** = triple Latitudes loyalty points. The code that replaces it, `LATREW`, is the baseline 1x-points marker, so the swap is a silent 3x→1x downgrade.
   - **`FREESRVC`** = "Free Pre-Paid Service Charges" (guests 1–2, Balcony and above; confirmed from a real NCL agent promo flyer). Worth roughly $20–25 per person per night — plausibly $300–500 on a real sailing, i.e. typically far more than the fare drop being traded for it. Real observed behavior: a reprice replaces it with a much smaller "$50 On-Board Credit Certificate."

   Both share the property that justifies a hard gate rather than a dollar subtraction: **no invoice line item to net off, and irreversible once repriced.** Implement as a **hard gate checked BEFORE any status is assigned** (so it can never surface as `OPTIMIZATION` no matter how large the price drop), producing `TRAP` with an explicit note naming *every* protected promo being lost (return a sorted list, not the first match) and the lowest confidence. Keep the protected set as a named collection — more codes will be added as they're observed. Parse promo strings tolerantly: the booking header is **comma**-separated while the category-grid row is **pipe**-separated. All other combinations (kept, gained, never present) are allowed.

   **Calibration check for the rebuild:** across four real bookings that ALL showed a genuine fare drop ($20/$60/$40/$72), a correct implementation flags **three as TRAP** (two for `FREESRVC`, one for `LATRIPLE`) and passes only one as a real `OPTIMIZATION`. If your implementation passes all four, the gate isn't wired in before status assignment.
...
- [ ] **`calculate_ncl` blocks any reprice that would lose `LATRIPLE` OR `FREESRVC`** (§5.2-CORRECTED point 7) — hard gate before status assignment, returns `TRAP` naming every protected promo lost, and correctly allows the kept/gained/never-present cases. Promo parsing handles both comma- and pipe-separated strings. Verify against the four-booking calibration set in that section: three must be TRAP, one OPTIMIZATION.
